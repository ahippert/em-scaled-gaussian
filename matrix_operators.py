...
def _matrix_operator(Ci, operator):
    ...
    eigvals, eigvects = sp.linalg.eigh(Ci, check_finite=False)
...
def inv(Ci):
        """Return the inverse matrix of a covariance matrix defined by :

        .. math::
                \mathbf{C} = \mathbf{V} \left( \mathbf{\Lambda} \\right)^{-1} \mathbf{V}^T
        ...
        """
        i_inv = lambda x: 1. / x
        return _matrix_operator(Ci, i_inv)
